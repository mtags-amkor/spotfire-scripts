...
sqlcmd -S%CONNECTIDENTIFIER% -U%ADMINNAME% -P%ADMINPASSWORD% -i create_actionlog_user_rds.sql -v ACTIONDB_NAME="%ACTIONDB_NAME%" ACTIONDB_USER="%ACTIONDB_USER%" ACTIONDB_PASSWORD="%ACTIONDB_PASSWORD%" >> actionlog.txt
if %errorlevel% neq 0 (
  @echo Error while running SQL script 'create_actionlog_user_rds.sql'
  @echo For more information consult the actionlog.txt file
  exit /B 1
)
