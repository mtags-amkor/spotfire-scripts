...
# Where should the data be stored on the database server
...
echo "Creating TIBCO Spotfire Server table spaces and user"
sqlplus ${ADMINNAME}/${ADMINPASSWORD}@${CONNECTIDENTIFIER} @create_server_env.sql  "${ROOTFOLDER}" "${SERVERDB_USER}" "${SERVERDB_PASSWORD}" "${SERVER_DATA_TABLESPACE}" "${SERVER_TEMP_TABLESPACE}" > log.txt
